...
def prepare_data(datadir, featdir, conf, feat_type, articulator=False, dynamic):
    '''
    ...
    #read the pos files
    path = datadir + '/pos'
    posfiles = [f for f in os.listdir(path) if os.isfile(join(path, f))]
    ...
    #Articulator features (Could be combined with the audio features below this)
    if articulator:
        art_comp = feat.FeatureComputer('art', dynamic, conf)
        for pos in posfiles:
            byt = read_pos(i)
            art_feat = art_comp(byt, 16000)
            # write to ark!!!!!!!!!!!
...
def read_pos(posfile):
    return np.fromfile(posfile, dtype='<f4')
