...
class GMM(object):
    ...
    def train(self):
        ...
        os.system('%s --cmd %s --config %s/config/%s %s %s %s %s %s' %(
            ...
            self.conf_file, self.trainops, self.conf.get('directories', 'train_features') + '/' + self.conf.get('gmm-features', 'name'),
    ...
    def align(self):
        ...
        for i in range(int(self.conf.get('general', 'num_jobs'))):
            os.system('''gunzip -c %s/ali/ali.%d.gz | /home/akshay/kaldi/src/bin/ali-to-pdf %s/ali/final.mdl ark:- ark,t:- | gzip >  %s/ali/pdf.%d.gz''' % (
                    self.conf.get('directories', 'expdir') + '/' + self.name,
...
class LdaGmm(GMM):
    ...
    @property
    def trainops(self):
        return '--context-opts "--context_width=%s" %s %s'% (
